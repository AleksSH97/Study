...
#define _CMD_GET_XYZ                "xyz"
#define _CMD_RESET_XYZ              "reset"
...
int console_execute(microrl_t *microrl_ptr, int argc, const char * const *argv) {
    ...
    while (i < argc) {
        ...
        else if (strcmp(argv[i], _CMD_GET_XYZ) == 0) {
            log_printf_crlf("\t%d", axis_spi.xyz_buf[0]);
            log_printf_crlf("\t%d", axis_spi.xyz_buf[1]);
            log_printf_crlf("\t%d", axis_spi.xyz_buf[2]);
        }
        else if (strcmp(argv[i], _CMD_RESET_XYZ) == 0) {
            accelerometer_reset();
        }
        else {
