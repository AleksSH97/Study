...
void lis302dl_init(uint16_t init_struct)
{
    ...
    axis_io_write(&ctrl, LIS302DL_CTRL_REG1_ADDR, 1);
}
/******************************************************************************/
...
uint8_t lis302dl_read_id(void)
{
    ...
    axis_io_read(&tmp, LIS302DL_WHO_AM_I_ADDR, 1);
...
void lis302dl_filter_config(uint8_t filter_struct)
{
    ...
    axis_io_read(&ctrl, LIS302DL_CTRL_REG2_ADDR, 1);
    ...
    axis_io_write(&ctrl, LIS302DL_CTRL_REG2_ADDR, 1);
}
/******************************************************************************/
...
void lis302dl_click_int_config(void)
{
    ...
    lis302dl_interrupt_config(&lis302dl_interrupt_conf);

    /* Configure Interrupt control register: enable Click interrupt on INT1 and
    INT2 on Z LIS302DL high event */
    ctrl = 0x3F;
    axis_io_write(&ctrl, LIS302DL_CTRL_REG3_ADDR, 1);

    /* Enable Interrupt generation on click on Z LIS302DL */
    ctrl = 0x50;
    axis_io_write(&ctrl, LIS302DL_CLICK_CFG_REG_ADDR, 1);

    /* Configure Click Threshold on X/Y LIS302DL (10 x 0.5g) */
    ctrl = 0xAA;
    axis_io_write(&ctrl, LIS302DL_CLICK_THSY_X_REG_ADDR, 1);

    /* Configure Click Threshold on Z LIS302DL (10 x 0.5g) */
    ctrl = 0x0A;
    axis_io_write(&ctrl, LIS302DL_CLICK_THSZ_REG_ADDR, 1);

    /* Enable interrupt on Y LIS302DL high event */
    ctrl = 0x4C;
    axis_io_write(&ctrl, LIS302DL_FF_WU_CFG1_REG_ADDR, 1);

    /* Configure Time Limit */
    ctrl = 0x03;
    axis_io_write(&ctrl, LIS302DL_CLICK_TIMELIMIT_REG_ADDR, 1);

    /* Configure Latency */
    ctrl = 0x7F;
    axis_io_write(&ctrl, LIS302DL_CLICK_LATENCY_REG_ADDR, 1);

    /* Configure Click Window */
    ctrl = 0x7F;
    axis_io_write(&ctrl, LIS302DL_CLICK_WINDOW_REG_ADDR, 1);
}
/******************************************************************************/
...
void lis302dl_interrupt_config(struct lis302dl_interrupt_conf *lis302dl_interrupt_conf_ptr)
{
    ...
    axis_io_read(&ctrl, LIS302DL_CLICK_CFG_REG_ADDR, 1);
...
void lis302dl_click_int_clear(void)
{
    ...
    axis_io_read(&clickreg, LIS302DL_CLICK_SRC_REG_ADDR, 1);
    axis_io_read(buffer, LIS302DL_STATUS_REG_ADDR, 6);
}
/******************************************************************************/
...
void lis302dl_reboot_cmd(void)
{
    ...
    axis_io_read(&tmpreg, LIS302DL_CTRL_REG2_ADDR, 1);

    tmpreg |= LIS302DL_BOOT_REBOOTMEMORY;

    axis_io_write(&tmpreg, LIS302DL_CTRL_REG2_ADDR, 1);
}
/******************************************************************************/
...
void lis302dl_read_acc(int16_t *pData)
{
    ...
    axis_io_read(&ctrl, LIS302DL_CTRL_REG1_ADDR, 1);
    axis_io_read((uint8_t*)buffer, LIS302DL_OUT_X_ADDR, 6);
    ...
    /* Obtain the mg value for the three LIS302DL */
